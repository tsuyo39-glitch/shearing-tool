...
class ProductRegistry:
    ...
    def list(self):
        ...
        try:
            return connection.execute("SELECT name,spec,thickness,width,length,qty,rotation FROM products ORDER BY name").fetchall()
        finally:
            connection.close()

    def save(self, values, quantity, rotation):
        ...
        try:
            with connection:
                connection.execute("INSERT OR REPLACE INTO products VALUES (?,?,?,?,?,?,?)",
                                   (name,spec,thickness,width,length,qty,int(rotation)))
        finally:
            connection.close()
