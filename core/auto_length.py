...
def optimize_auto(products, width=1219.0, edge=10.0, max_length=2438.0,
                  gap=0.0, time_limit=10.0, cancel=None, length_loss=0.0):
    values = (width, edge, max_length, gap, time_limit, length_loss)
    if not all(math.isfinite(v) for v in values):
        raise ValueError("設定には有限の数値を入力してください。")
    if edge < 0 or gap < 0 or length_loss < 0 or width <= edge or max_length <= 2*length_loss or time_limit <= 0:
        raise ValueError("幅は幅ロスより大きく、最大長さ・計算時間は正の数、切断代・ロスは0以上にしてください。")
    ...
    groups = {}
    for p in products:
        groups.setdefault((p.spec, p.thickness), []).append(p)
    ...
    for strategy in range(24):
        ...
        for (spec, thickness), items in groups.items():
            inner = SheetType("inner", "有効寸法", spec, thickness, width-edge,
                              max_length-2*length_loss, edge_loss=0, cut_allowance=gap)
    ...
    return sorted(candidates, key=lambda r: (not r.complete, sum(r.shortage_by_product.values()),
                  sum(s.sheet_type.area for s in r.sheets), len(r.sheets)))[:10]
